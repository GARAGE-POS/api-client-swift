# ProductReportDto

## Properties
Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**itemID** | **Int64** | Item/Product ID | 
**SKU** | **String** | Product SKU | 
**productName** | **String** | Product name | 
**itemSold** | **Double** | Total quantity sold (minus refunded quantity) | 
**grossSales** | **Double** | Gross sales (Quantity × UnitPrice, before discounts) | 
**netSales** | **Double** | Net sales (Gross - Line Discounts - Allocated Order Discounts + Taxes) | 
**refund** | **Double** | Total refund amount for this product | 
**discount** | **Double** | Total discount (line item discount + allocated order discount) | 
**taxes** | **Double** | Total tax amount | 
**profit** | **Double** | Item profit (Net Sales - Cost of Goods Sold) Calculated as: NetSales - (UnitCost × ItemSold) | 

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


